...
\begin{center}
\rtl{rtl/datapath/rtl/exe\_stage/}
\end{center}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.55\textwidth]{ch09_execution_overview.png}
...
\midrule
\endhead
\rtl{alu\_add.sv} & Addition and subtraction \\
\rtl{alu\_logic.sv} & AND, OR, XOR operations \\
\rtl{alu\_shift.sv} & Logical and arithmetic shifts \\
\rtl{alu\_cmp.sv} & Comparison instructions (SLT, SLTU, etc.) \\
\rtl{alu\_count\_pop.sv} & Population count instructions \\
\rtl{zero\_counter/alu\_count\_zeros.sv} & Leading/trailing zero count operations \\
\bottomrule
\end{longtable}
...
The dedicated Address Generation Unit (\rtl{vagu.sv}) computes effective memory addresses
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{ch09_execution_pipeline_flow.png}
...
\begin{rtlrefs}
  ...
  \rtlitem{pending_fp_ops_queue.sv}
  \rtlitem{score_board_scalar.sv}
  \rtlitem{score_board_simd.sv}
\end{rtlrefs}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.55\textwidth]{ch09_execution_detail.png}
